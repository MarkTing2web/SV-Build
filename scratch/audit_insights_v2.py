"""
audit_insights_v2.py — Securevision Insights Audit Script
Scope:  /insights/*.html  (article pages + index.html hub)
Action: READ ONLY — no file modifications. Produces markdown report.
Run:    python audit_insights_v2.py
Output: insights-audit-report-v2.md  (same dir as this script, or set OUT_FILE below)
"""
...
OUT_FILE    = r"C:\Projects\SV-Build\insights-audit-report-v2.md"
# ─────────────────────────────────────────────────────────────────────────────
...
def check_file(filepath):
    ...
    if not bc:
        ...
        # FIX v1: last item check — get last child element, check it's not an <a>
        bc_items = [el for el in bc.children if el.name]
        if bc_items:
            last = bc_items[-1]
            # look for last element that contains text — could be a <li>, <span>, or <a>
            last_a_in_bc = bc.find_all("a")
            all_text_nodes = bc.find_all(["span", "li", "a"])
            if all_text_nodes and all_text_nodes[-1].name == "a":
    ...
    if re.search(r'Level\s*3', html) and "sv-bizsafe" not in html:
        add_issue(filepath, "A10.5", "bizSAFE Level 3 appears hardcoded (no sv-bizsafe class found)")
    ...
    if not is_hub:
        # Prev/next
        prev_next = soup.find(class_=re.compile(r"prev.next|prev-next"))
        if not prev_next:
    ...
    for el in styled_els:
        st = (el.get("style") or "").lower()
        # Only hero background-image on <style> in <head> is allowed — any style= on elements is a violation
        add_issue(filepath, "A14.1", f"style= attribute on <{el.name}> (class: {' '.join(get_classes(el))[:40]})")
        if any(k in st for k in ("font-size", "font-family", "font-weight", "color:")):
            add_issue(filepath, "A14.2", f"Inline font/color on <{el.name}>")
        if any(k in st for k in ("padding", "margin", "gap:", "display:")):
            add_issue(filepath, "A14.3", f"Inline layout style on <{el.name}>")

    # FIX v1: A14.1 is now reported per element above, not just once
    # Remove duplicate A14.1 from older logic (already handled above)
...
if __name__ == "__main__":
    ...
    out.append("## Securevision SV-Web · Generated by audit_insights_v2.py")
    out.append("")
    ...
    out.append("---")
    out.append("")
    out.append("*Securevision · Insights Audit · v2.0 · June 2026*")
