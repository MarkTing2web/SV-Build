...
OUT_FILE    = r"C:\Projects\SV-Build\insights-audit-report-v6.md"
# ─────────────────────────────────────────────────────────────────────────────
...
def check_file(filepath):
    ...
    for cb in soup.find_all(class_=re.compile(r"callout")):
        classes = get_classes(cb)
        is_container = "callout-box" in classes
        # Skip child elements (e.g. .callout-label) that live inside a .callout-box
        has_box_ancestor = any("callout-box" in (a.get("class") or []) for a in cb.parents)
        if not is_container and not has_box_ancestor:
            add_issue(filepath, "A9.1", f"Callout element uses wrong class: {' '.join(classes)}")
        if cb.get("style") and not has_box_ancestor:
            add_issue(filepath, "A9.5", "Inline style on callout element")

    for vb in soup.find_all(class_=re.compile(r"verdict")):
        classes = get_classes(vb)
        is_container = "verdict-box" in classes
        # Skip child elements (e.g. .verdict-label) that live inside a .verdict-box
        has_box_ancestor = any("verdict-box" in (a.get("class") or []) for a in vb.parents)
        if not is_container and not has_box_ancestor:
            add_issue(filepath, "A9.2", f"Verdict element uses wrong class: {' '.join(classes)}")
        if vb.get("style") and not has_box_ancestor:
...
if __name__ == "__main__":
    ...
    out.append("## Securevision SV-Web · Generated by audit_insights_v6.py")
    out.append("")
    ...
    out.append("---")
    out.append("")
    out.append("*Securevision · Insights Audit · v6.0 · June 2026*")
