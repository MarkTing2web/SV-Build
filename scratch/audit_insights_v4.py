"""
audit_insights_v4.py — Securevision Insights Audit Script
Scope:  /insights/*.html  (article pages + index.html hub)
Action: READ ONLY — no file modifications. Produces markdown report.
Run:    python audit_insights_v4.py
Output: insights-audit-report-v4.md  (same dir as this script, or set OUT_FILE below)

Changes from v2:
  FIX A14  — Per-element rows replaced with per-file summary (count + tag list).
             Prevents report flooding on pages with many inline styles.
             A14.2/A14.3 now report unique element types, not per-element rows.
  FIX A13.1 — Regex dot in prev.next escaped; added article-nav as alternate class.
  FIX A10.5 — Removed full-page Level 3 scan; trust bar already checked in A5.6.
             Articles legitimately mention "bizSAFE Level 3" in prose.
  FIX A6.5  — Breadcrumb last-item check rewritten to handle both list and inline
             breadcrumb markup patterns reliably.
"""
...
OUT_FILE    = r"C:\Projects\SV-Build\insights-audit-report-v4.md"
# ─────────────────────────────────────────────────────────────────────────────
...
def check_file(filepath):
    ...
    if not is_hub and not soup.find(class_=re.compile(r"related")):
        add_issue(filepath, "A3.6", "Related articles section missing")
    ...
    for cb in soup.find_all(class_=re.compile(r"callout")):
        if "callout-box" not in get_classes(cb) and not cb.find_parent(class_="callout-box"):
    ...
    for vb in soup.find_all(class_=re.compile(r"verdict")):
        if "verdict-box" not in get_classes(vb) and not vb.find_parent(class_="verdict-box"):
    ...
    # Strip out article body before checking for hardcoded sites
    html_to_check = html
    ab = soup.find("div", class_="article-body")
    if ab:
        html_to_check = html.replace(str(ab), "")
    
    if re.search(r'\b[12][,\s]?\d{3}\+?\s*sites', html_to_check.lower()) and "sv-sites" not in html:
        add_issue(filepath, "A10.3", "sv-sites appears hardcoded outside article body")
    ...
    if not is_hub:
        ...
        if not rel_section:
            add_issue(filepath, "A13.2", "Related articles section missing")
        else:
            rel_links = rel_section.find_all("a", href=True)
            if len(rel_links) < 2:
                add_issue(filepath, "A13.4", f"Related section has only {len(rel_links)} link(s) — minimum 2 required")
...
if __name__ == "__main__":
    ...
    out.append("## Securevision SV-Web · Generated by audit_insights_v4.py")
    out.append("")
    ...
    out.append("---")
    out.append("")
    out.append("*Securevision · Insights Audit · v4.0 · June 2026*")
