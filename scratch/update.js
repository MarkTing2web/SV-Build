...
// We will read from the checked-out version again to be clean.
// Or we can just read the current file. Since it might have duplicates now, we will filter properly.
// Let's do a fresh checkout to avoid compound errors.
const { execSync } = require('child_process');
execSync('git checkout site-config.js', { cwd: 'C:\\Projects\\SV-Build' });

let content = fs.readFileSync('C:\\Projects\\SV-Build\\site-config.js', 'utf8');

// Task 1: Fix authorTitle
content = content.replace("authorTitle: 'Founder & CEO, Securevision Pte Ltd',", "authorTitle: 'Founder & Director, Securevision Pte Ltd',");

// Task 2: Remove 18 orphan entries
const orphans = [
...
// Extract slugs from new41
let newSlugs = [];
let matches = new41.matchAll(/slug:\s*"([^"]+)"/g);
for (const match of matches) {
    newSlugs.push(match[1]);
}
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (insideArray) {
        // Extract slug from current line
        let m = line.match(/slug:\s*"([^"]+)"/);
        if (m) {
            let slug = m[1];
            if (orphans.includes(slug)) {
                removedOrphanCount++;
                continue; // drop orphan
            }
            if (newSlugs.includes(slug)) {
                removedDuplicateCount++;
                continue; // drop duplicate because it will be added in new41
            }
            validCount++;
            newLines.push(line);
        } else {
            // Probably a comment or empty line inside the array
            newLines.push(line);
        }
    } else {
        newLines.push(line);
...
console.log("Removed Orphans: " + removedOrphanCount);
console.log("Removed Duplicates (to be replaced): " + removedDuplicateCount);
console.log("Valid existing entries kept: " + validCount);

content = newLines.join('\n');

const parts = content.split('SECUREVISION.guides = [');
let beforeGuides = parts[0];

const lastBracketIdx = beforeGuides.lastIndexOf('}');
const endIdx = beforeGuides.indexOf('];', lastBracketIdx);

let beforeLastBracket = beforeGuides.substring(0, lastBracketIdx + 1);
if (!beforeGuides.substring(lastBracketIdx + 1, endIdx).includes(',')) {
    beforeLastBracket += ',';
}

beforeGuides = beforeLastBracket + '\n' + new41 + '\n' + beforeGuides.substring(endIdx);

const finalContent = beforeGuides + 'SECUREVISION.guides = [' + parts[1];

fs.writeFileSync('C:\\Projects\\SV-Build\\site-config.js', finalContent);

// Verification count
let verifyMatch = finalContent.match(/SECUREVISION\.insights\s*=\s*\[([\s\S]*?)\];/);
if (verifyMatch) {
    let arrayContent = verifyMatch[1];
    let slugs = arrayContent.match(/slug:/g);
    console.log("Total entries in SECUREVISION.insights = " + (slugs ? slugs.length : 0));
}
